package com.zhengshun.touch.api.common.util.generator;
	...
	public static List<Table> getList(String dbClass, String table_name, String url, String MysqlUser,
		...
		try {
			...
			while (rs.next()) {
				...
				table.setTypeName(StringUtil.lowerCase(rs
						.getString("COLUMN_NAME")));
				table.setColumnName(StringUtil.uncapitalize(toUpper(StringUtil
						.lowerCase(rs.getString("COLUMN_NAME"))))); // 将所有的字段名改为小写方便命名
				table.setColumnNameUpper(StringUtil.capitalize(table
						.getColumnName()));
